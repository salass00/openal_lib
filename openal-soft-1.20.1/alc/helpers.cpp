...
const PathNamePair &GetProcBinary()
{
    ...
    IDOS->DevNameFromLock(IDOS->GetProgramDir(), progdir, sizeof(progdir), DN_FULLPATH);

    if (!IDOS->GetCliProgramName(progname, sizeof(progname)))
        strlcpy(progname, IExec->FindTask(NULL)->tc_Node.ln_Name, sizeof(progname));

    IDOS->AddPart(progdir, IDOS->FilePart(progname), sizeof(progdir));

    pathlen = strlen(progdir);
    pathname.resize(pathlen);
    std::memcpy(pathname.data(), progdir, pathlen);
#endif
#ifdef HAVE_PROC_PIDPATH
    if(pathname.empty())
    {
